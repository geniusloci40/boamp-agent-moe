...
def send_email(tenders):
    ...
    if count == 0:
        body = "<p style='color:#666'>Nessun appalto MOE pertinente pubblicato ieri.</p>"
    else:
        cards = ""
        for t in tenders:
...
<p style="margin:0 0 4px;font-size:13px;color:#888;">{t.get('dateparution','')} · {t.get('lieuexecution','—')}</p>
